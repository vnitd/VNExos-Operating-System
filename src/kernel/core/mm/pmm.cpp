...
void init_memory(memmap_entry *mm, size_t size)
{
    ...
    for (int32_t i = 0; i < size; i++)
    {
        ...
        {
            free_mem_regions[free_region_count].address = mm[i].base;
            free_mem_regions[free_region_count].length  = mm[i].length;
            free_region_count++;
        }
    }
    ...
    {
        uint64_t vstart = P2V(free_mem_regions[i].address);
        uint64_t vend   = vstart + free_mem_regions[i].length;
...
void kfree(uint64_t v)
{
    ...
    mem_page *page_address = (mem_page *)v;
...
void *kalloc(void)
{
    mem_page *page_address = free_mem.next;
