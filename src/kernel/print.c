...
static void write_screen(const char *buffer, int size, struct ScreenBuffer *sb,
                         char color)
{
        ...
        else
        {
            switch (format[++i])
            {
                case 'x':
                    ...
                    buffer_size +=
                        hex_to_string(buffer, buffer_size, (uint64_t)integer);
                    break;

                case 'u':
                    integer = va_arg(args, int64_t);
                    buffer_size += udecimal_to_string(buffer, buffer_size,
                                                      (uint64_t)integer);
                    break;
                ...
                case 'd':
                    integer = va_arg(args, int64_t);
                    buffer_size +=
                        decimal_to_string(buffer, buffer_size, integer);
                    break;
